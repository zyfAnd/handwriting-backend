/**
 * Cloudflare Worker - 汉字手写体图片搜索 API
 *
 * 功能：
 * - 单字/多字搜索
 * - 返回手写体图片URL列表
    ...
    if (isChineseChar(char)) {
      ...
      if (charData) {
        results.push({
          ...
          unicode: charData.unicode || `U+${char.charCodeAt(0).toString(16).toUpperCase().padStart(4, '0')}`,
...
function constructImageUrl(char, env) {
  ...
  // 使用R2公开域名
  const domain = env.R2_PUBLIC_DOMAIN || 'handwriting-characters.r2.dev';
...
<body>
  <h1>🖌️ 汉字手写体图片搜索 API</h1>

  <p>欢迎使用汉字手写体图片搜索API！本API提供常用汉字的手写体图片。</p>
